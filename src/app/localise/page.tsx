...
import VariantList, { type LocalisedVariant } from "@/components/localise/VariantList";
...
export default function LocalisePage() {
  ...
  useEffect(() => {
    ...
    if (savedVariants) {
      try {
        ...
        setVariants(parsed.map((v) => ({ ...v, audioUrl: undefined, audioLoading: false })));
      } catch { /* ignore */ }
    }
    setLoaded(true);
  }, []);
  ...
  useEffect(() => {
    if (variants.length > 0) {
      const toSave = variants.map((v) => ({ ...v, audioUrl: undefined, audioLoading: false }));
      localStorage.setItem(STORAGE_KEYS.LOCALISED_VARIANTS, JSON.stringify(toSave));
    }
  }, [variants]);

  const synthesiseAudio = useCallback(async (scriptText: string, voiceId: string): Promise<string> => {
    ...
    if (!res.ok) {
      ...
      throw new Error((data as { error?: string }).error ?? "Audio synthesis failed.");
    }
  ...
  const handleLocalise = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/localise", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ baseScript: confirmedScript.body, regionIds: selectedRegionIds }),
      });
      if (!res.ok) {
        const data = await res.json().catch(() => ({}));
        throw new Error((data as { error?: string }).error ?? "Localisation failed.");
      }
      ...
      setLocaliseError(err instanceof Error ? err.message : "Localisation failed.");
    } finally {
      setIsLocalising(false);
    }
  }, [confirmedScript, selectedRegionIds]);

  const handleGenerateAudio = useCallback(async (regionId: string) => {
    if (!selectedVoiceId) return;
    const variant = variants.find((v) => v.regionId === regionId);
    if (!variant?.localisedScript) return;
    setVariants((prev) => prev.map((v) => v.regionId === regionId ? { ...v, audioLoading: true } : v));
    try {
      const url = await synthesiseAudio(variant.localisedScript, selectedVoiceId);
      setVariants((prev) => prev.map((v) => v.regionId === regionId ? { ...v, audioUrl: url, audioLoading: false } : v));
    } catch (err) {
      setVariants((prev) => prev.map((v) => v.regionId === regionId ? { ...v, audioLoading: false, error: err instanceof Error ? err.message : "Audio generation failed." } : v));
    }
  }, [variants, selectedVoiceId, synthesiseAudio]);

  const handleGenerateAll = useCallback(async () => {
    if (!selectedVoiceId) return;
    const pending = variants.filter((v) => !v.audioUrl && !v.error && v.localisedScript);
    ...
    for (let i = 0; i < pending.length; i++) {
      ...
      setVariants((prev) => prev.map((v) => v.regionId === variant.regionId ? { ...v, audioLoading: true } : v));
      try {
        const url = await synthesiseAudio(variant.localisedScript, selectedVoiceId);
        setVariants((prev) => prev.map((v) => v.regionId === variant.regionId ? { ...v, audioUrl: url, audioLoading: false } : v));
      } catch (err) {
        setVariants((prev) => prev.map((v) => v.regionId === variant.regionId ? { ...v, audioLoading: false, error: err instanceof Error ? err.message : "Audio generation failed." } : v));
      }
      if (i < pending.length - 1) await new Promise((resolve) => setTimeout(resolve, 600));
    }
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-8">
        <div className="rounded-2xl border border-dashed border-[#27272a] p-12 text-center max-w-md bg-[#18181b]">
          ...
          <p className="text-white font-semibold mb-1">No confirmed script found</p>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-400">
                <span className="font-bold text-white">{selectedRegionIds.length}</span> Regions
              </span>
            </div>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-400">
                <span className="font-bold text-white">{totalStations}</span> Stations
              </span>
            </div>
            <div className="w-px h-4 bg-[#3a3a3a]" />
            <div className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-gray-400">
                <span className="font-bold text-white">{totalReach > 0 ? `${(totalReach / 1_000_000).toFixed(1)}M` : "-"}</span> Listeners
              </span>
            </div>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          ...
          {variants.length === 0 && (
            <button
              ...
              style={{ background: "linear-gradient(to right, #6f42c1, #8a2be2)" }}
            >
              {isLocalising ? (
                <>
          ...
          {localiseError && (
            <div className="mt-4">
              <ErrorBanner message={localiseError} onDismiss={() => setLocaliseError(null)} onRetry={handleLocalise} />
            </div>
          )}
          ...
          {!selectedVoiceId && (
            <div className="mt-4 flex items-start gap-3 px-4 py-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
              <Icon name="warning" className="text-xl text-amber-400 shrink-0 mt-0.5" />
              <div>
                <p className="text-sm text-amber-300 font-medium">No voice selected</p>
                <p className="text-xs text-amber-400/70 mt-0.5">
                  Select a voice on the{" "}
                  <Link href="/voice" className="underline">Voice page</Link>{" "}
              ...
              <div className="bg-[#18181b] border border-[#27272a] rounded-xl p-4">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <Icon name="hub" className="text-lg text-[#a78bfa]" />
                    <h2 className="text-white font-bold text-sm">Regional Adaptations</h2>
                  </div>
                ...
                <p className="text-xs text-gray-500 leading-relaxed">
                  Each script has been rewritten for its local market, weaving in regional landmarks, cultural references, and conversational tone that resonates with local listeners. Purple-highlighted words show localised changes.
                </p>
              </div>
